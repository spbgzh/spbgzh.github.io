...
public class RectangularSpliterator implements Spliterator.OfInt {
    ...
    public RectangularSpliterator trySplit() {
        ...
        if(this.content.length * this.content[0].length <= 1) return null;
        ...
        if(content[0].length <= content.length) {
            ...
            for (int i = 0; i < this.content.length / 2; i++) {
                for (int j = 0; j < this.content[0].length; j++) fhalf[i][j] = content[i][j];
            }
            shalf = new int[(this.content.length + 1) / 2][this.content[0].length];
            for (int i = this.content.length / 2; i < this.content.length; i++) {
                for (int j = 0; j < this.content[0].length; j++) shalf[i - this.content.length / 2][j] = content[i][j];
            }
        }
        else {
            fhalf = new int[this.content.length][this.content[0].length / 2];
            for (int i = 0; i < this.content.length; i++) {
                for (int j = 0; j < this.content[0].length / 2; j++) fhalf[i][j] = content[i][j];
            }
            shalf = new int[this.content.length][(this.content[0].length+1)/2];
            for (int i = 0; i < this.content.length; i++) {
                for (int j = this.content[0].length / 2; j < this.content[0].length; j++) shalf[i][j - this.content[0].length / 2] = content[i][j];
            }
        }
        this.content = shalf;
    ...
    public boolean tryAdvance(IntConsumer action) {
        ...
        if(itercon.length > 0) {
            ...
            for(int y=0; y<itercon.length - 1; y++) tmp[y] = itercon[y + 1];
            itercon = tmp;
